...
- **기준 플랜:** `~/.claude/plans/editorial-magazine-studio-virtual-nova.md`(v11 전 단계 완료·배포). 다음: v11 오너 육안 — `verify-shots/title-v11-*.png` 11장은 **단색 플레이스홀더**라 구조만 보인다. 실사진으로 반투명 카드가 사진을 충분히 살리는지 / 기본 1.25배가 과한지(과하면 `collageTokens TITLE_BASE_SCALE`) / 가로 배치·투명 배경이 쓸 만한지. 이전 플랜: `~/.claude/plans/breezy-juggling-key.md`(v10). 이전 잔여: v10 육안 — `verify-shots/collage-v10-*.png` 10장은 **단색 플레이스홀더**라 구조만 보인다. 실사진으로 ① 세 템플릿이 충분히 다르게 느껴지는지 ② **매거진 18장에서 히어로가 보드 절반을 먹는 게 "주인공"으로 읽히는지 과한지**(과하면 `collageTokens heroMaxWFor`의 매거진 0.6을 낮춘다) ③ 기본 스티커 킷이 도움인지 거슬림인지 ④ 모바일 드래그 스왑·크게/작게 조작감. 이전 잔여(v8.6 MergeSheet·/logged-out 육안, v8.5 육안 5건, 실계정 복원 확인, Vercel Web Analytics)
- **주요 기능:**
...
- **알려진 이슈:** v11 배포로 기존 사용자의 타이틀 카드가 반투명·1.25배로 **한 번 바뀐다**(의도된 기본값 변경 — 시트에서 되돌릴 수 있다). 편집 모드에서 타이틀 **글자**가 덮은 자리의 사진은 탭이 안 된다 — 타이틀을 옮기면 된다(카드 여백·투명 영역은 통과한다). 로그인 사용자는 v10 첫 진입 때 병합 시트를 **한 번** 볼 수 있다(마이그레이션이 로컬 보드를 바꾸는데 서버 보드는 아직 v4 모양 — "이 기기 것으로 계속"을 고르면 된다. 병합 로직 자체는 무손상, verify-v86r1 16/16). 에디토리얼이 풀블리드라 보드에 '빈 곳'이 없다 — 편집 진입의 결정적 동선은 상시 `✎ 탭해서 편집` 버튼뿐이다. hydration #418은 전 페이지 공통 useState(loadBoard()) 패턴의 기존 이슈(표시는 정상). 폰 뷰를 PC에서 열면 세로 스크롤이 남는다 — 9:19.5 보드를 ≥300px로 유지하면서 1280×900에 넣는 건 기하학적으로 불가(보드만 649px+크롬 383px). PC 뷰는 무스크롤. 로그인 사용자의 원격 사진 표시가 우리 함수/대역폭을 경유한다(immutable CDN 캐시로 URL당 사실상 1회 — 대역폭 모니터 필요). public에 미사용 한글 파일명 에셋 잔존(정리 대상). verify-v614의 팔레트 대비 정적 검사는 비비드 복원으로 FAIL이 정상.

## 세션 로그
<!-- ⚠️ APPEND ONLY — 아래 항목을 절대 삭제/수정하지 마세요. 새 항목은 이 줄 바로 아래에 추가합니다. -->

### 2026-08-13 (v11 — 콜라주 타이틀 커스터마이즈, 프로덕션 배포 완료)
...
- 배포: `npx vercel --prod`(GitHub 자동배포 없음) → prod 6스위트 재검증 PASS — v11r1 41/41 · v10r1 30/30 · v87r1 41/41 · v85r1 32/32 · v81r2 33/33 · 스모크 20/20
