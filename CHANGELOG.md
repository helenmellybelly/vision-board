...
- **상태:** v6.12 사용자 피드백 6건 반영 완료 — **프로덕션 미배포, 사용자 로컬 확인·피드백 대기 중**
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조, 온보딩 전체 뷰포트 고정(`h-dvh`, 페이지 스크롤바 없음)
  - 온보딩 Act 2 채팅: 내부 스크롤 + 자동 하단 + `.scroll-soft`(호버 시만 표시되는 스크롤바)
  - 디자인 시스템 v6.12: 선명한 섹션 6색 — `lib/colors.ts` 단일 소스(#6F56C9/#27804F/#A8600D/#356FBE/#B54E20/#187A8C, 4.5:1 검증), 틴트 카드→흰 카드+컬러 좌측 보더
  - 용어: "장면 그리기" → "미래의 하루 그리기" (ProcessBar·가이드·리뷰·scene 등 라벨 전체)
  - `/collage` 배경화면 저장: 한 장 모아담기(18장 겹침 콜라주) / 섹션 묶음 3장, Canvas 직접 렌더(1170×2532) + Web Share/다운로드 폴백, 외부 이미지 프록시 API
  - 접근성: 전역 focus-visible 링, 모달 포커스 트랩(`useFocusTrap`), Gowun Batang 디스플레이 서체
- **알려진 이슈:** v6.12 미배포 (`npx vercel --prod`는 사용자 명시 요청 필요, GitHub 푸시 자동 배포 없음). public에 미사용 한글 파일명 에셋 잔존(정리 대상). 667px 이하 소형 화면에서 Act 4는 내부 소프트 스크롤 발생(콘텐츠는 전부 도달 가능).
