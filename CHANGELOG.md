...
- **상태:** v8.4 배경화면 내보내기 복구·콜라주 배열 개선·완주 카피·/diary 탭·스토리 프롬프트 고도화 — **프로덕션 라이브** (커밋 5402772, 2026-07-31). 오너 피드백 6건 해결: ① "사진 N장 못 불러옴" 근본 수정 — 프록시 허용 호스트에 자기 Blob 호스트(`*.public.blob.vercel-storage.com`) 추가(로그인 후 Blob URL의 CORS 실패+프록시 403이 원인)+loadOne 타임아웃+"다시 시도", ② 저장 위치 픽커(showSaveFilePicker 우선, 클릭 첫 await·공유/다운로드 폴백·미리보기 캔버스 재사용), ③ 숲 레거시 가시성 가드(`isLayoutBroken` — edited 플래그 없는 pre-v8.0 겹침 배치만 리시드), ④ 미니멀 균형 행 분배(`layoutBalancedGrid` — 고아 행 원천 제거, seed·reflow 공유), ⑤ 완주 대시보드 "완성해도 끝은 아니야" 카피(연도 완료형 삭제)+미래 일기 읽기 보더 필 승격, ⑥ /diary 전체+섹션 7탭(탭 전환·빈 섹션 안내), ⑦ 스토리 프롬프트 v2(사람 먼저·현실감 마찰·오글 금지 열거·하루의 결 랜덤 주입·문장 리듬)+클리셰 게이트(`lib/honorific hasCliche`, 본편·섹션 공용 1회 재생성)+Groq 폴백 8B→70B+storyPromptVersion 재작성 넛지. verify-collage-layout 352 + verify-v84r1 신설 28 + 갱신 5스위트(v83r1·r2b·v612~614·smoke-v80-story S8) + 로컬 25스위트 전건 PASS, prod 스모크 20/20+33/33+28/28+스토리 8/8
- **기준 플랜:** `~/.claude/plans/whimsical-finding-origami.md`(v8.4, 완료). 다음 대기: 오너 육안 4건(배경화면 저장 skipped=0+위치 대화상자·숲/미니멀 배열·/diary 탭·넛지 재생성 품질), 이전 잔여(온보딩 로그인 실계정 복원 확인·Vercel Web Analytics·매거진형 템플릿(선택))
- **주요 기능:**
