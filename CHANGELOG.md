...
- **상태:** v8.3 완주 경험·재방문 로그인 — **프로덕션 라이브** (커밋 0dc8ce0, 2026-07-31). 오너 피드백 5건(P1 신규 기기 로그인 진입 부재, P2 일기 링크 2개 혼란, P3 완주 후 📷 잔존, P4 연도 라인 진행형+편집 소음, P5 완주 전후 무변화) 해결: ① `/diary` 통합 열람 신설(미래의 하루 이야기 대표+섹션 일기 부록, DiarySheet 모달 폐기, 대시보드 일기 진입점 1원화), ② 완주 대시보드 정리(📷 행·연도 편집 숨김, "그린 보드야" 완료형), ③ 온보딩 셸 3스텝 공통 "Google로 이어가기"+choice 재방문자 캡션(복원은 기존 AccountFlow 전역 병합이 담당 — 트리거만 추가), ④ 완주 연출(다 자란 참나무 2배+글로우 탭→/collage, FOREST.gradientDoneCss 노을 변주, 첫 완주 잎 파티클 1회 finishCelebrated 스탬프·reduced-motion 가드), ⑤ api/story 마무리 변주 지시+S7 소제목 가드. verify-v83r1 신설 32 + 갱신 3스위트(v79·v81r1·smoke-v81-prod) + 로컬 22스위트 전건 PASS, prod 스모크 20/20 + prod v83r1 32/32
- **기준 플랜:** `~/.claude/plans/glowing-bouncing-sutherland.md`(v8.3, 완료) + 기획 `docs/완주경험-리플래닝-v83.md`. 다음 대기: 오너 육안 3건(완주 연출·/diary 위계·온보딩 로그인 실계정 복원), 오너 보드 복구 2건(섹션1 /scenes 저장 1회 재완료·깨진 핀 사진 교체), Vercel Web Analytics, 매거진형 템플릿(선택)
- **주요 기능:**
