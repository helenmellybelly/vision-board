...
- **상태:** v6.7 프로덕션 배포 완료 (master 4252f2e) — 영상 교체·온보딩 모바일 최적화·Act 4 스와이프·보드 재설계·스토리 수정
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조
  - 온보딩 Act 0: 캐릭터 영상 `tori-final2.mp4` (최종2.mp4 → 720p CRF28, 172KB)
  - 온보딩 Act 1~5: 토리 프로필+이름 헤더 패턴 통일, 채팅·입력 전폭 (모바일 저장버튼 꾸겨짐 해결)
  - 온보딩 Act 4: 막연함(앉은 모습·흑백)↔선명함(러닝) 스와이프 카드 + 텍스트 오버레이
  - /board: 섹션 CTA 상시 표시 + 상태별 라우팅 (`lib/sectionRoute.ts` 공용), 스토리 팝업(`StoryModal`), 미래하루 스토리 카드/힌트
  - /board 웹(md+): 좌 섹션 업로드 / 우 sticky 한눈에 보기 2-pane (Collage `compact` prop), 모바일 세로 유지
  - 스토리 직접 수정: 보드·moment에서 miniStory/futureDayStory 편집, finish 재생성 시 덮어쓰기 확인
  - QA: Playwright 검증 (이번 세션 28체크, 375px/1280px)
- **알려진 이슈:** 없음. ⚠️ 배포는 GitHub 푸시로 자동 트리거 안 됨 — `vercel --prod` CLI 필수
