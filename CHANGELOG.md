...
- **상태:** v7.5 R1 산책길 대시보드 + 콜라주 2탭 — **구현·로컬 검증 완료, 프로덕션 배포 대기**(사용자 확인 후 `npx vercel --prod`). 회귀 14스위트 319/319 PASS(verify-v75r1 27케이스 신설, v618b 아카이브). 회원가입 기획서 v7.5 개정(온보딩 종료 선택 화면 + 첫 사진 소프트 게이트 + 동의 2종 + 운영 가이드) — 구현은 R2
...
- **주요 기능:**
  ...
  - 대시보드 = 산책길 허브 (v7.5, `components/WalkPathMap.tsx`): CTA 묶음(추천 카드·내 비전보드 보기·사진부터)이 타이틀 바로 아래, 그 밑에 도토리 출발→6스테이션(🌰/📷/✍️/🌳+완료 칩, 사진 썸네일 미표시)→참나무 언덕 세로 지그재그 지도 + 토리 마커. aria-label 계약 `${label} — ${STATUS_LABEL}` 유지(MiniBoardPreview에서 export 공유, collage/review/finish의 미니보드는 존치). 진행 카피 "N/6 스테이션을 지났어" + **연도 캡션·스테퍼 편집(targetDate 연동)** + **"질문 없이, 사진부터 담아볼래?" 시트(/scenes 직행)**
  - `/collage` (v7.5): 기본 뷰 PC, **[폰|PC] 2탭**(보드 탭 제거 — `?view=board` 레거시는 desktop으로 정규화, 완성 CTA·이야기 블록은 기기 뷰 공통 노출) + **사이즈 칩 상시 노출(탭 즉시 적용·프리셋 자동 시드)**, `?view=` URL 동기화(`?device=` 레거시 호환), 템플릿 3종 드래그 편집 + 기기 프리셋 11종
  - 하루 일기 (v7.3): 사용자 장면 중심 프롬프트(고정 아크·체크리스트 제거, 고유 단어·감각 디테일 규칙, 450-650자), scene 작성 가이드 카드 + 6섹션 연결형 예문
...
### 2026-07-23 (v7.5 R1 — 산책길 대시보드 + 콜라주 2탭 + 회원가입 기획서 개정, 배포 대기)
