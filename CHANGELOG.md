...
- **기준 플랜:** `~/.claude/plans/pc-dynamic-wreath.md`(v8.7 P0 완료 / Part 2 = v8.8 UI·UX·버그헌트·보안감사 대기). 다음 대기: 오너 육안(실계정 로그인 후 배경화면 저장 skipped 0·칩 배치·PC 무스크롤), 이전 잔여(v8.6 MergeSheet·/logged-out 육안, v8.5 육안 5건, 실계정 복원 확인, Vercel Web Analytics, 매거진형 템플릿(선택))
- **주요 기능:**
  ...
  - `/collage`: 전 뷰포트 [폰|PC] 2탭(lg 기본 PC·좁은 화면 기본 phone, v8.2)+보드 전폭+sticky 저장 바+폰 뷰 lg 컨트롤 레일, 시드 cols 최적 탐색·리사이즈 리플로우·사진 탭 라이트박스(v8.2), 편집 모드 출처 섹션 배지+사진 인라인 교체/삭제+깨진 사진 ⚠️(v8.1), 템플릿 3종('숲'(구 폴라로이드, id는 polaroid 유지)·모자이크·미니멀) — **v7.6 프레임리스 라운드 사진 단일 경로**(DOM/canvas 락스텝), 스티커 프리셋 18종(이모지 chip/script 한정), 기기 프리셋 11종
