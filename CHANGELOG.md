...
- **상태:** v6.14 피드백 5건 반영 — **푸시·프로덕션 배포 완료** (커밋 0d2105c, https://vision-board-web.vercel.app 4경로 200 확인)
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조, 온보딩 전체 뷰포트 고정, 3뷰포트(375×667/390×844/1280×720) 무스크롤
  - 디자인 시스템 v6.14 (`docs/design-system.md`): 타입 스케일 7토큰 + **text-display↔font-display 페어링 규칙** (`scripts/check-typography.js` 기계 검증)
  - 팔레트: v6.14 선명 톤(채도↑) — `lib/colors.ts` 단일 소스(#7757D9/#15814E/#A86208/#2B71C9/#C24A1F/#0A7C8B, 크림 4.52~4.81)
  - `/board`: 2열×3행 무스크롤 (모바일 포함 3뷰포트), 스토리는 헤더 아이콘 팝업
  - `/collage`: 템플릿 4종(폴라로이드/모자이크/미니멀/내 배치) + **드래그·리사이즈 배치 편집**(겹침 허용, localStorage 저장)
  - 배경화면 저장: 모드 2종 × 디자인 2종 × **타깃 2종(모바일 1170×2532 / PC 1920×1080)**
- **알려진 이슈:** 커스텀 배치는 배경화면 렌더에 미반영(다음 버전 과제). public에 미사용 한글 파일명 에셋 잔존(정리 대상).
