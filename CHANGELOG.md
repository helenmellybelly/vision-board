...
- **기준 플랜:** `~/.claude/plans/1-valiant-meerkat.md`(v9.0 전 단계 완료). 다음 대기: 오너 육안(`verify-shots/collage-v90-desktop-{6,12,18}.png`를 실제 PC 배경화면으로 적용해 "왁!" 해소 확인 / 배경색 8종이 실사진과 어울리는지 — 특히 다크 2색에서 모자이크·미니멀 사진 경계 / 모바일 스왑·크게·작게 손맛 / 매트 갤러리 무크롭), 이전 잔여(v8.6 MergeSheet·/logged-out 육안, v8.5 육안 5건, 실계정 복원 확인, Vercel Web Analytics)
- **주요 기능:**
  ...
  - `/collage` (v9.0 그리드 재설계): 전 뷰포트 [폰|PC] 2탭+보드 전폭+sticky 저장 바, 템플릿 3종(**모자이크**=매거진 리듬·히어로 허용·행마다 넓은 칸 회전 / **미니멀**=절제·히어로 없음 / **매트 갤러리**=넓은 매트·균일 1×1·무크롭) + **배경색 팔레트 8색 공통**. 배치는 `GridSpec`이 진실이고 좌표는 파생(`lib/collageGrid.ts`), 조판 수치는 `lib/collageTokens.ts` 단일 소스. 편집: 이동=스왑(스팬은 '자리'에 귀속)·리사이즈=스팬 등급 1..4 스냅·탭 액션 크게/작게·자유 배치 토글(여기서만 회전). 편집 모드 출처 섹션 배지+사진 인라인 교체/삭제+깨진 사진 ⚠️(v8.1), 스티커 프리셋 18종, 기기 프리셋 11종
...
- **알려진 이슈:** hydration #418은 전 페이지 공통 useState(loadBoard()) 패턴의 기존 이슈(표시는 정상). 폰 뷰를 PC에서 열면 세로 스크롤이 남는다 — 9:19.5 보드를 ≥300px로 유지하면서 1280×900에 넣는 건 기하학적으로 불가(보드만 649px+크롬 383px). PC 뷰는 무스크롤. 로그인 사용자의 원격 사진 표시가 우리 함수/대역폭을 경유한다(immutable CDN 캐시로 URL당 사실상 1회 — 대역폭 모니터 필요). public에 미사용 한글 파일명 에셋 잔존(정리 대상). verify-v614의 팔레트 대비 정적 검사는 비비드 복원으로 FAIL이 정상.
