...
- **상태:** v7.4 R1 리플래닝 라운드 — **프로덕션 배포 완료** (커밋 b67d75b 감사 + cbc733a R1, https://vision-board-web.vercel.app 11경로 200 + /moment 404 + 번들 신규 카피 7종 확인, 회귀 13스위트 290/290 PASS, 프로덕션 일기 Gemini 실생성 확인)
...
- **주요 기능:**
  ...
  - 대시보드 = 정원 맵 허브 (v7.2→v7.3): 미니보드 씨앗🌱/새싹🌿 + 뱃지 3종(✓/✍️/📷) + 토리 마커, 정원 캡션, 추천 카드 문장형, '내 비전보드 보기' 단일 버튼 + **연도 캡션·스테퍼 편집(targetDate 연동)** + **"질문 없이, 사진부터 담아볼래?" 시트(/scenes 직행)**
  - `/collage` (v7.3): 기본 뷰 PC, [보드|폰|PC] 토글 + **사이즈 칩 상시 노출(탭 즉시 적용·프리셋 자동 시드)** + **모든 뷰 저장(보드 뷰 4:5 포함)**, `?view=` URL 동기화(`?device=` 레거시 호환), 템플릿 3종 드래그 편집 + 기기 프리셋 11종
