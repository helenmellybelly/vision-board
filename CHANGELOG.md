...
- **상태:** v6.6 프로덕션 배포 완료 (master c38a64f) — 캐릭터 영상 교체·Act 3/4 재구성·폴라로이드 콜라주·모바일 최적화
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조
  - 온보딩 Act 0: 새 캐릭터 영상 `tori-final.mp4` (최종.mp4 → 720p CRF28, 211KB, 베이지 배경)
  - 온보딩 Act 3: 비전보드 정의 + 실제 예시 이미지 (모바일 세로형 / 웹 가로형 반응형 전환)
  - 온보딩 Act 4: 막연함vs선명함 + 새벽 러닝 사진 + "비전보드를 하면 좋은 이유" 카드 3개
  - 채팅: 토리 프로필 이미지 아바타, 흰 배경 입력창
  - /board: 섹션별 업로드 + 하단 "한눈에 보기" 폴라로이드 콜라주 (연도 탭 수정 → localStorage)
  - 모바일: h-dvh 채팅 레이아웃, safe-area 하단 패딩, 반응형 말풍선
  - QA: `scripts/verify-v66.mjs` Playwright 2뷰포트 스크린샷 검증
