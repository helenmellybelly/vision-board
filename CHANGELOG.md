...
- **상태:** v6.5 구현 완료 (로컬 검증 통과, 배포 대기) — 새 캐릭터 영상·도토리 탭 전용·예시 목업·자유 업로드
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조
  - 온보딩 Act 0: 새 캐릭터 영상 (인사_0610-2 원본 colorkey=black 변환, 560×560 WebM 434KB)
  - 온보딩 Act 2 도토리: 탭 전용 진행 (자동 타이머 제거)
  - 온보딩 Act 3: 완성 비전보드 미리보기 목업 (6영역 그리드 + Unsplash) + CTA "와, 기대되는데? →"
  - 상단 진행 표시: 라벨형 ChapterProgress → 미니멀 점 5개
  - /board: 질문 답변 없이 빈 슬롯 탭 → 바로 사진 업로드 가능, 대시보드 진입 버튼 상시 노출
  - 섹션 채팅: 도입 2개 메시지 + 질문 1개 말풍선(cushion+question 통합)
  - AI 이미지 3장 구도 다양성, 통합 5슬롯 갤러리
- **알려진 이슈:** 없음 (눈 하얘짐 해결 — 망가진 배경제거본 대신 검은 배경 원본 사용)
