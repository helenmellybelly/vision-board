...
- **상태:** v8.1 수정 플로우 정상화·완주 후 상태·이미지 신뢰성·콜라주 나란히+인라인 편집 — **프로덕션 라이브** (커밋 cc21ea3+ace348b, 2026-07-30). 오너 실기기 피드백 5건 해결: ① 무변경 '수정'이 completed 강등·LLM 재검증·일기 재생성 유도하던 3중 결함 제거(강등 가드+dirty 게이트+선택지 배너), ② 완주 후 대시보드 "완성한 내 비전보드 보기" CTA, ③ /scenes URL 이미지 검증 게이트(핀 페이지 주소 전용 카피), ④ 콜라주 lg+ 좌 PC·우 폰 나란히(좁은 화면 기본 phone)+편집 모드 출처 섹션 배지+사진 인라인 교체/삭제+깨진 사진 ⚠️, ⑤ 로그인 시 답변만 있는 보드 유실 가드(merge). 로컬 verify-v81r1 35+v81r2 32+갱신 구 스위트 128+prod 스모크 19/19
- **리플래닝 기준 문서:** `~/.claude/plans/structured-wishing-liskov.md`(v8.1, 완료 — Ultraplan 클라우드 구현 인수). 다음 대기: 오너 보드 복구 2건(섹션1 /scenes 저장 1회 재완료·깨진 핀 사진 교체), 나란히 레이아웃 실기기 육안, 실 OAuth 기기 간 이어하기, Vercel Web Analytics, 매거진형 템플릿(선택)
- **주요 기능:**
  ...
  - `/collage`: lg+ 좌 PC·우 폰 나란히 실렌더(좁은 화면은 [폰|PC] 2탭·기본 phone), 편집 모드 출처 섹션 배지+사진 인라인 교체/삭제+깨진 사진 ⚠️(v8.1), 템플릿 3종('숲'(구 폴라로이드, id는 polaroid 유지)·모자이크·미니멀) — **v7.6 프레임리스 라운드 사진 단일 경로**(DOM/canvas 락스텝), 스티커 프리셋 18종(이모지 chip/script 한정), 기기 프리셋 11종
