...
- **상태:** v8.2 콜라주 뷰 분리 복원·이미지 극대화·리사이즈 리플로우·라이트박스 — **프로덕션 라이브** (커밋 9053f96, 2026-07-31). 오너 피드백("PC·폰 동시 노출로 이미지가 너무 작다") 해결: ① v8.1 나란히 철회→전 뷰포트 폰/PC 2탭+보드 전폭(PC 1104px, 18장 최소 셀 56→103px), ② 저장 버튼 sticky 하단 바+높이 예산 CSS 변수(--board-reserve)+폰 뷰 lg 좌 스테이지·우 컨트롤 레일, ③ 시드 cols 최적 탐색(1장 히어로 37%~18장 자동 적응, k-축소 갭 제외 교정), ④ 리사이즈 후 전체 리플로우(그리드 정합 시, 숲 제외, 220ms 정착 모션), ⑤ 감상 모드 사진 탭→원본 확대(공용 Lightbox, scenes 통합)·배경 탭→편집 유지. 기하 186+v82r1 17+v81r2 재작성 33+회귀 12스위트 전건 PASS, prod 스모크 20/20+prod v82r1 17/17
- **기준 플랜:** `~/.claude/plans/floating-swinging-russell.md`(v8.2, 완료). 다음 대기: 오너 실기기 육안(2탭·리플로우·라이트박스), 오너 보드 복구 2건(섹션1 /scenes 저장 1회 재완료·깨진 핀 사진 교체), 실 OAuth 기기 간 이어하기, Vercel Web Analytics, 매거진형 템플릿(선택)
- **주요 기능:**
