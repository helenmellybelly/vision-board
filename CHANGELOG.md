...
- **상태:** v6.11 커밋 완료 (master 27142fc + 003e843) — **프로덕션 미배포, 사용자 로컬 확인·피드백 대기 중**
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조
  - 온보딩 Act 0: 진짜 알파 채널 `tori-v3-alpha.webm` 우선 + `tori-v3.mp4`(multiply) 폴백
  - 디자인 시스템 v6.11: Gowun Batang 디스플레이 서체(`.font-display`), 정원 톤 섹션 6색(라벤더·세이지·허니·청회·테라코타·호수), 정보성 텍스트 `#6E6962`(WCAG 4.5:1+)
  - 랜딩 히어로: 토리 + 6영역 폴라로이드 보드(순차 등장) — 콜라주 룩 미리보기, 카루셀도 폴라로이드 프레임
  - 접근성: 전역 focus-visible 링, 모달 3종 포커스 트랩+Esc+ARIA(`useFocusTrap`), 채팅 본문 모바일 15px
  - 아이콘: lucide-react — UI 이모지 전부 SVG/토리 이미지로 교체 (카피 속 이모지는 유지)
  - 대시보드: 완성 섹션 폴라로이드 썸네일, ProcessBar 5단계
- **알려진 이슈:** v6.11 미배포 (`npx vercel --prod`는 권한상 사용자 명시 요청 필요). public에 미사용 한글 파일명 에셋 잔존(정리 대상). ⚠️ 배포는 GitHub 푸시로 자동 트리거 안 됨 — CLI 필수.
