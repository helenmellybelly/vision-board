...
- **알려진 이슈:** 로그인 사용자는 v10 첫 진입 때 병합 시트를 **한 번** 볼 수 있다(마이그레이션이 로컬 보드를 바꾸는데 서버 보드는 아직 v4 모양 — "이 기기 것으로 계속"을 고르면 된다. 병합 로직 자체는 무손상, verify-v86r1 16/16). 에디토리얼이 풀블리드라 보드에 '빈 곳'이 없다 — 편집 진입의 결정적 동선은 상시 `✎ 탭해서 편집` 버튼뿐이다. hydration #418은 전 페이지 공통 useState(loadBoard()) 패턴의 기존 이슈(표시는 정상). 폰 뷰를 PC에서 열면 세로 스크롤이 남는다 — 9:19.5 보드를 ≥300px로 유지하면서 1280×900에 넣는 건 기하학적으로 불가(보드만 649px+크롬 383px). PC 뷰는 무스크롤. 로그인 사용자의 원격 사진 표시가 우리 함수/대역폭을 경유한다(immutable CDN 캐시로 URL당 사실상 1회 — 대역폭 모니터 필요). public에 미사용 한글 파일명 에셋 잔존(정리 대상). verify-v614의 팔레트 대비 정적 검사는 비비드 복원으로 FAIL이 정상.
