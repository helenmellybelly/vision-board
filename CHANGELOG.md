...
- **상태:** v6.13 디자인 시스템 정립 + 피드백 6건 반영 완료 — **프로덕션 미배포, 사용자 로컬 확인·피드백 대기 중** (localhost:3000 기동 상태로 종료)
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조, 온보딩 전체 뷰포트 고정, Act 4 포함 3뷰포트(375×667/390×844/1280×720) 무스크롤
  - 디자인 시스템 v6.13 (`docs/design-system.md`): 타입 스케일 7토큰(globals.css @theme — display-lg 32 ~ micro 11), 폰트 규칙(말풍선=Pretendard, Gowun Batang=디스플레이 전용)
  - 팔레트: v6.11 뮤트 톤 복귀+대비 보정 — `lib/colors.ts` 단일 소스(#7868A9/#4F7A5F/#996826/#5273A3/#B05A36/#3D7B87, 크림 4.5:1+)
  - `/board`: 데스크톱 3열 그리드(1280×720 무스크롤), 모바일 세로 스택 유지
  - `/collage` 배경화면 저장: 모드 2종(모아담기/섹션 묶음) × **디자인 2종(폴라로이드 다크/미니멀 크림)**
  - 접근성: 전역 focus-visible 링(텍스트 입력은 보더 전환으로 대체), 모달 포커스 트랩
- **알려진 이슈:** v6.13 미배포 (`npx vercel --prod`는 사용자 명시 요청 필요, GitHub 푸시 자동 배포 없음). master가 origin보다 ahead(미푸시). public에 미사용 한글 파일명 에셋 잔존(정리 대상).
