...
- **상태:** v6.15 피드백 7건 반영 — **푸시·프로덕션 배포 완료** (커밋 156665b, https://vision-board-web.vercel.app 5경로+폰트 200 확인)
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — Acts 0-5 구조, 온보딩 전체 뷰포트 고정, 3뷰포트(375×667/390×844/1280×720) 무스크롤
  - 디자인 시스템 v6.15 (`docs/design-system.md`): 타입 스케일 7토큰 + 간격 스케일 + **UI Pretendard 단일 / 아트 Enjoystories(.font-script, collage 전용)** (`scripts/check-typography.js` 기계 검증)
  - 팔레트: v6.13 파스텔/뮤트 톤 복원 — `lib/colors.ts` 단일 소스(#7868A9/#4F7A5F/#996826/#5273A3/#B05A36/#3D7B87, 크림 4.56~4.66)
...
- **알려진 이슈:** 콜라주 템플릿 비주얼은 사용자 샘플 기준 추가 조정 여지(피드백 대기). public에 미사용 한글 파일명 에셋 잔존(정리 대상).
