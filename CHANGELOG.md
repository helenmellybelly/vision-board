...
- **상태:** v6.3-story 코드 구현 완료 — Acts 0-5 Tori 도토리 스토리텔링 온보딩 적용 (feat/tori-rebrand 브랜치)
- **주요 기능:**
  - 🐿️ 토리 캐릭터 (꿈의 정원사) — float CSS 애니메이션 적용
  - 온보딩 Acts 0-5: Tori 첫인사(정원사) → 도토리 이야기(2.5cm→60m 2,400배, 6단 말풍선) → 버킷리스트(유/무 분기) → 감정연결 → 정원진단 → 6개 화단 안내
  - Tori 시스템 프롬프트: 도토리/참나무 세계관 직접 전달, 정원사 정체성, 감정 곡선
  - ChapterProgress 컴포넌트: 5개 챕터 하단 진행 바
