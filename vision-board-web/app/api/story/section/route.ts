...
- 감정 단어 나열
- 한국어가 아닌 문자 — 일본어(히라가나·가타카나)·한자·키릴 문자를 단 한 글자도 섞지 말 것
...
[그려낸 장면]이 하루의 중심이 되게.`;
  ...
  try {
    ...
    // 품질 게이트는 /api/story와 공용 경로 lib/storyGate (v8.5) — 존댓말·클리셰·외국 문자
    // 검출 시 1회 재생성 후 재검증. 같은 산출물 계열은 검증 경로를 공유해야 품질 격차가 안 생긴다
