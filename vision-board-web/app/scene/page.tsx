    ...
    <main className="min-h-screen flex flex-col max-w-md mx-auto w-full px-6 py-10">
      <div className="flex items-center gap-3 mb-8">
        <button
          ...
          className="p-2 -ml-2 text-[#6B7280] text-title active:opacity-60"
        >
          ‹
        </button>
        <div>
          <h1 className="text-title font-bold">미래의 하루 그리기</h1>
      ...
      <p className="text-body text-[#6B7280] mb-5 leading-relaxed">
      ...
      <div className="space-y-3 flex-1">
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-body">{section.title.split(' — ')[0]}</p>
                    {keyword && (
                      <p className="text-caption mt-0.5" style={{ color: section.color }}>
                        키워드: {keyword}
                      </p>
                    )}
                    {sceneText ? (
                      <p className="text-caption text-[#6E6962] mt-0.5 line-clamp-1">{sceneText}</p>
                    ) : (
                      <p className="text-caption text-[#C4C2BE] mt-0.5">아직 그리기 전이야</p>
                    )}
                  </div>
                </div>
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  {isDone ? (
                    ...
                    <span className="text-caption px-2 py-0.5 rounded-full bg-[#FEF9C3] text-[#D97706] font-medium">
                      사진 남음
                    </span>
                  ) : (
                    <span className="text-caption px-2 py-0.5 rounded-full bg-[#F3F4F6] text-[#6B7280] font-medium">
                      그리기 전
                    </span>
                  )}
                  <span className="text-[#6E6962] text-body">›</span>
                </div>
              </div>
            </button>
          );
        ...
        {availableSections.length === 0 && (
          <div className="text-center py-16">
            <p className="text-[#6E6962] text-body">
              섹션 답변을 먼저 완료해야 해.
            </p>
            <button
              onClick={() => router.push('/dashboard')}
              className="mt-4 text-body font-semibold underline text-[#6B7280]"
            >
              섹션으로 돌아가기
            </button>
          </div>
