...
### 사용자가 "기능이 없다"고 말할 때, 있는데 안전하지 않아서 안 쓴 경우를 먼저 의심할 것 #coding #ux
"배치가 박스에 갇혀 있다"는 신고를 받고 보니 자유 배치 토글은 3개 버전 전부터 있었다 — 안 쓴 이유는 끄는 순간 좌표가 복구 불가로 사라져서였다.
같은 세션에서 교체·삭제 동선도 이미 있는데 힌트가 없어 아무도 못 찾았다. 신고를 기능 요청으로 받기 전에 **이미 있는지 / 왜 안 쓰는지**(위험한가, 안 보이는가)를 먼저 확인하면 만들 것이 훨씬 줄어든다.

### 진행 중인 사용자 입력이 있으면 props 재동기화를 건너뛸 것 #coding #react
드래그 중 `resolveLayout` 재동기화를 막는 가드는 있었는데 텍스트 편집 중에는 없어서, 부모가 한 번만 리렌더해도 방금 만든 초안이 통째로 날아가 **아예 입력이 안 됐다**.
"사용자가 손대고 있는 상태를 props에서 되돌려 받으면 작업이 사라진다"는 드래그와 편집에 똑같이 적용된다 — 가드 조건에 편집 상태도 함께 넣을 것.

### 저장 시점에 청소하는 로직은 방금 만든 초안까지 지운다 #coding #storage
빈 스티커를 로드 시 정리하는 sweep을 넣었더니, 새 스티커를 만들자마자 저장 → 로드 → sweep이 돌아 **초안이 만들어지는 즉시 사라졌다**(툴바조차 안 떴다).
"미완성 상태를 정리하는 규칙"과 "미완성 상태로 시작하는 UI"는 충돌한다 — 초안은 저장하지 않고 로컬 상태로만 들고 있다가 완성 시점에 저장하는 쪽이 근본 해법이다.
...
### 서버를 띄워둔 채 빌드하면 무관한 스위트가 무더기로 FAIL한다 #coding #testing #next-js
E2E가 도는 중에 `npm run build`를 돌리면 `.next` 산출물이 교체돼 청크 불일치로 클라이언트가 조용히 깨진다 — 이번엔 v71r4·v75r1·v81r1·v82r1이 한꺼번에 빨개졌다가 재실행에서 전부 PASS였다(내가 만진 파일과 무관한 실패라 원인을 오판하기 딱 좋다).
빌드는 스위트가 끝난 뒤에만, 그리고 빌드했으면 **포트 소유자 PID를 죽이고 새로 띄운 뒤** 이번 변경의 새 시그니처로 새 빌드인지 확인하고 돌릴 것.

### 결과를 끝에 한 번만 출력하는 러너는 길게 돌 때 스위트별 append로 바꿔 돌릴 것 #coding #testing
`run-all-*.mjs`처럼 결과를 배열에 모아 마지막에 한 번 출력하는 러너는 중간에 죽으면(타임아웃·강제 종료) **진행 상황이 전부 유실**된다 — 15분짜리 회귀를 두 번 날렸다.
길게 도는 회귀는 스위트마다 결과 파일에 append하는 루프로 감싸 부분 진행을 보존하고, 실패한 스위트만 재실행할 수 있게 할 것.

### 새 컨트롤의 접근 이름이 기존 컨트롤 이름을 부분 문자열로 포함하면 기존 스위트가 깨진다 #coding #testing #a11y
"사진 바꾸기·지우기"라는 aria-label을 붙였더니 `getByRole('button', {name:'사진 바꾸기'})`가 18개 배지와 액션 칩을 한꺼번에 집어 strict mode 위반이 났다(여는 쪽과 열리는 쪽의 이름이 겹친 것).
메뉴를 여는 컨트롤과 그 안의 액션은 접근 이름을 겹치지 않게 짓고, 기존 라벨을 바꿀 땐 `aria-label`을 유지해 셀렉터 계약을 보존할 것.
