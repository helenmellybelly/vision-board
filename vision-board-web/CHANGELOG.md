...
- **상태:** v5.0 프로덕션 라이브 — moment/scenes URL 분리 완료
- **주요 기능:**
  - 섹션 채팅(lumi) → 장면 → 순간(`/moment/[id]`) → 장면카드+이미지(`/scenes/[id]`)
  - `/moment/[id]`: situation 입력 + 스토리 생성 (2단계)
  - `/scenes/[id]`: 묘사 3카드 (편집/슬롯별 재생성) + 이미지 업로드/AI생성 + 이미지 없이도 저장 가능
  - `api/image/describe-one`: 장면 단일 묘사 재생성 (기존 묘사 컨텍스트 활용)
  - 대시보드: 완성 섹션에 이미지 스트립(h-20) + 스토리 프리뷰 + 이미지 클릭 라이트박스
- **알려진 이슈:** 없음
