...
- **상태:** v12 콜라주 편집 자율성 4커밋(fec3771·8d556ae·a0e8077·36b2de3) — **프로덕션 배포·푸시 완료**, 로컬 34/34 스위트 PASS + prod 6스위트 225케이스 PASS
- **주요 기능:**
  - 문구 인라인 편집: 시트 폐기, 글자를 보드 위에서 직접 수정(contentEditable) + 보드 안 플로팅 툴바(프리셋·스타일·색·↵줄바꿈·크기·삭제). 줄바꿈 개방(40자 1줄 → 120자/6줄), ✥ 전용 이동 핸들
  - 축하 화면: `/finish`가 실제 보드 렌더러(BoardPreview→BoardCanvasDom) 사용 — 구 숲 6칸 그리드는 `SectionProgressGrid`로 개명해 `/scenes` 진행 표시 전용
  - 자유 배치 왕복 무손실: `CollageLayout.freeItems` 스태시(스키마 v5 유지) + 라벨 🔓 자유롭게 옮기기 / 🔒 다시 정렬(aria-label '자유 배치' 유지)
  - 사진 발견성: 편집 모드 사진마다 `⋯` 진입점(기존 setPhotoAction 재사용) — 교체·삭제가 3단계 무힌트였던 문제 해소
  - 단일 소스화: 스티커 조판 수치가 `lib/collageTokens`(stickerHeightNorm·wrapStickerText) 하나로, DOM 렌더러가 `BoardCanvasDom` 하나로 수렴
  - 검증: `scripts/verify-sticker.js` 84 신설(run-all-v86 등록) + `.claude/verify-v12r1.mjs` 48 신설
- **알려진 이슈:** hydration #418 기존 전역 패턴(표시 정상). ⚠️ 빈 문구 스티커는 로드 시 자동 정리되므로 기존 유령 항목이 첫 진입에 사라진다. 오너 육안 2건(⋯ 배지 밀도·실기기 한글 IME 줄바꿈) 미확인

## 세션 로그
<!-- ⚠️ APPEND ONLY — 아래 항목을 절대 삭제/수정하지 마세요. 새 항목은 이 줄 바로 아래에 추가합니다. -->

### 2026-08-13 (v12 — 콜라주 편집 자율성: 인라인 문구·축하 화면 실제 보드·자유 배치 왕복·사진 발견성)
- 오너 피드백 4건의 **원인이 전부 증상과 달랐다**: ① 줄바꿈은 렌더러가 아니라 입력(`<input type=text>` 40자)이 막고 있었고 ② "추가해도 안 늘어남"은 `newStickerLayoutItem`이 항상 같은 좌표를 반환해 포개진 것(+추가 경로가 `{items,stickers}`만 저장해 spec·title·aspect를 잃던 2차 버그) ③ 자유 배치는 이미 있었지만 끄면 좌표가 복구 불가로 사라져 아무도 안 쓴 것 ④ `/finish`의 숲 레이아웃은 마이그레이션 누락이 아니라 축하 화면이 애초에 실제 렌더러를 안 쓴 것
- 구조적 뿌리 2개 제거: "스티커 높이"를 4곳이 각자 계산(둘은 오답 — chip 실높이가 정사각 가정의 0.26배라 **보드 하단 20%에 문구를 놓을 수 없었다**) → `stickerHeightNorm` 단일 소스 / 보드를 그리는 코드 3벌 → `BoardCanvasDom` 하나로. 부수로 canvas가 긴 한국어를 글자 분해할 때 글자 사이 공백을 끼우던 결함도 수정
- 육안·실사용으로만 잡힌 결함 9건 추가 수정(폰 폭에서 툴바 라벨 전부 2줄 깨짐·빈 문구 유령이 사진 탭 가로챔·프롭 재동기화가 초안을 날림·폰↔PC 전환 시 문구 유실·`✎` 버튼 실높이 8px·⋯ 배지 접근 이름이 액션 칩과 충돌). 로컬 34/34 + prod 6스위트(v12r1 48·v11r1 41·v10r1 30·v87r1 41·v85r1 32·v81r2 33) PASS 후 `npx vercel --prod` 배포
