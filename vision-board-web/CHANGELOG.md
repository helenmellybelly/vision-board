...
- **상태:** v6.0 온보딩 리브랜드 완료 — lumi → 🐿️ 토리(꿈의 정원사), 정원 메타포 UI 전반에서 제거 (정원사 아이덴티티만 유지)
- **주요 기능:**
  - 온보딩 4단계: 토리 등장 → 버킷리스트+상상하기 → 이름+상태 체크 → 진입
  - 정원사 토리 아이덴티티: 전체 UI에서 ✦ lumi → 🐿️ 토리 교체
  - 질문 콘텐츠: cushionText/introText/whyText 원문 복원 (정원/화단 메타포 제거)
  - 방문 페이지/대시보드/보드/완료/리뷰/웰컴에서 정원/화단 워딩 제거
- **알려진 이슈:** 없음
