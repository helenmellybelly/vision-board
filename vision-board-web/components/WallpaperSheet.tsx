...
export default function WallpaperSheet({ groups, year, onClose }: Props) {
  ...
  const key = mode === 'all' ? 'all' : `pair-${pairIdx}`;

  // 미리보기 생성 — 모드/슬라이드별 1회만
  useEffect(() => {
    ...
    (async () => {
      try {
        const canvas =
          mode === 'all'
            ? await renderAllInOne(groups, year)
            : await renderSectionPair(pairs[pairIdx], year);
        if (!cancelled) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await saveCanvas(canvas, `vision-board-${year}${suffix}.png`);
    } catch {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 id="wallpaper-title" className="text-base font-bold mb-1">
          배경화면으로 저장
        </h2>
        <p className="text-xs text-[#6E6962] mb-4">
        ...
        <div className="relative flex items-center justify-center">
          {previews[key] ? (
            ...
            >
              <span className="text-xs text-[#6E6962] animate-pulse">만드는 중...</span>
            </div>
          )}
        ...
        {mode === 'pairs' && pairs.length > 0 && (
          <div className="mt-3 flex flex-col items-center gap-1.5">
            ...
            <p className="text-[11px] text-[#6E6962]">{pairLabel(pairs[pairIdx])}</p>
          </div>
        )}

        {error && <p className="text-xs text-[#B91C1C] mt-3 text-center">{error}</p>}

        <button
          onClick={handleSave}
          disabled={saving || !previews[key]}
          className="mt-5 w-full py-4 rounded-2xl text-base font-semibold text-white disabled:opacity-40 transition-opacity active:opacity-80"
          style={{ backgroundColor: '#1C1B19' }}
        >
          {saving ? '저장 중...' : '이미지로 저장'}
        </button>
        <button onClick={onClose} className="mt-2 w-full py-2 text-sm text-[#6E6962]">
          닫기
        </button>
      </div>
    </div>
